...
class scheduleGenerator:
    ...
    def _init_today_schedule(self):
        weekday = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][datetime.now(global_config.time_zone).weekday()]
    ...
    def get_current_task(self):
        ...
        if self.date != cur_date:
            if self._init_today_schedule():
                self.date = cur_date
        stamp = datetime.now(global_config.time_zone).hour * 60 + datetime.now(global_config.time_zone).minute
