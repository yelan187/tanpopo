...
logger = register_logger("schedule generator", global_config.log_level)
...
class ScheduleGenerator:
    ...
    def _init_today_schedule(self):
        weekday = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][
            datetime.now(global_config.time_zone).weekday()
        ]
        resp = self.llm_api.send_request_text(
            f"""<information>{global_config.bot_config['personality']}</information><task>你昨天的日程安排是f{self.today_schedule_text}。今天是{weekday}，"""
            + """你可以参考昨天的日程生成今天的日程安排，但尽量有变化。请按照时间顺序列出具体时间点和对应的活动，格式为{"开始时间": "活动","开始时间": "活动",...}，注意只需要给出开始时间而不是一个时间段。请用活动概括整个时间段需要进行的事件，活动名称不要出现准备/开始等词语，最后一项活动固定为睡觉。用JSON格式返回日程表，仅返回内容，不要返回注释，不要添加任何markdown或代码块样式，时间采用24小时制。</task>"""
        )
        try:
            raw = json.loads(resp)
            schedule = []
            ...
            self.yesterday_schedule = (
                self.today_schedule if self.today_schedule else schedule
            )
    ...
    def get_current_task(self):
        cur_date = datetime.now(global_config.time_zone).day
        if self.date != cur_date:
            if self._init_today_schedule():
                self.date = cur_date
        stamp = (
            datetime.now(global_config.time_zone).hour * 60
            + datetime.now(global_config.time_zone).minute
        )
        ...
        for i in range(len(self.today_schedule) - 1):
            if (
                self.today_schedule[i].start_time
                <= stamp
                < self.today_schedule[i + 1].start_time
            ):
